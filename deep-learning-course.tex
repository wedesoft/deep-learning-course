...
Given a context word $c$, a positive target $t$ (label $y=1$) in the vincinity is identified
and $k$ random negative examples for $t$ are also selected (label $y=0$).
...
$f$ is larger for frequent words.
...
Here the bilingual evaluation understudy (Bleu) score can be used.
...
Note that blank and space are different characters.
