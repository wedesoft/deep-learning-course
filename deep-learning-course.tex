...
\subsection{Different formulations}
\subsubsection{State-action rewards}
One can have a reward function $R:S\times A->\mathbb{R}$ which depends on the state as well as the action.
With state-action rewards the Bellman equation becomes
\begin{equation}
  V^\ast(s)=\max_a R(s,a)+\gamma\sum_{s^\prime}P_{sa}(s^\prime)V^\ast(s^\prime)
\end{equation}

\subsubsection{Finite horizon MDPs}
A finite horizon MDP is defined as $MDP(S,A,\{P^{(t)}_{sa}\},T,R)$ where $T$ is the horizon time.
The total payoff is $R^{(0)}(s_0,a_0)+R^{(1)}(s_1,a_1)+\ldots+R^{(T)}(s_T,a_T)$.
In this case the optimal policy is non-stationary (changes with time).
The transition probability may also depend on time: $s_{t+1}\sim P_{sa}^{(t)}$ (\emph{e.g.} changing dynamics of airplane with decreasing fuel).
The optimal value function is
\begin{equation}
  V^\ast_t(s)=E[R^{(t)}(s_t,a_t)+\ldots+R^{(T)}(s_T,a_T)|\Pi^\ast,s_t=s]
\end{equation}
The Bellman equation is
\begin{equation}
  \begin{split}
    V^\ast_T(s)&=\max_a R^{(T)}(s,a)\\
    V^\ast_t(s)&=\max_a R^{(t)}(s,a)+\sum_{s^\prime}P_{sa}^{(t)}(s^\prime)V^\ast_{t+1}(s^\prime)
  \end{split}
\end{equation}
The optimal policy is
\begin{equation}
  \Pi^\ast_t(s)=\mathop{\operatorname{argmax}}_a R^{(t)}(s,a)+\sum_{s^\prime}P_{sa}(s^\prime)V^\ast_{t+1}(s^\prime)
\end{equation}

\subsubsection{Linear Quadratic Regulation (LQR)}
The following assumption is very common and allows you to handle very large MDPs.
An MDP is defined using $S$, $A$, $\{P_{sa}\}$, $T$, and $R$.
Here $S=\mathbb{R}^n$, $A=\mathbb{R}^d$.
The transition probability is
\begin{equation}
  P_{sa}^{(t)}:s_{t+1}=A_ts_t+B_ta_t+w_t
\end{equation}
and the reward function is
\begin{equation}
  R^{(t)}(s_t,a_t)=-(s_t^\top U_ts_t+a_t^\top V_t a_t)
\end{equation}
where $w_t\sim\mathcal{N}(0,\mathcal{E}_w)$, $A_t,U_t\in\mathbb{R}^{n\times n}$, and $B_t,V_t\in\mathbb{R}^{n\times d}$.
$U_t$ and $V_t$ are positive semi-definite.
\emph{E.g.} a helicopter hovering where $s_t\approx 0$ is desired:
Choose $U_t=\mathcal{I}$, $V_t=\mathcal{I}$, $R(s_t,a_t)=-s_t^\top s_t-a_t^\top a_t=-||s_t||^2-||a_t||^2$.

For understanding the material one can assume $A=A_1=A_2=\ldots$ and $B=B_1=B_2=\ldots$.

\subsubsection{Coming up with a Linear Model}
To come up with a linear model, one can observe sequences
\begin{equation}
  \begin{split}
    s^{(1)}_0&\xrightarrow{a^{(1)}_0}s^{(1)}_1\xrightarrow{a^{(1)}_1}\ldots\rightarrow s^{(1)}_T\\
             &\vdots\\
    s^{(m)}_0&\xrightarrow{a^{(m)}_0}s^{(m)}_1\xrightarrow{a^{(m)}_1}\ldots\rightarrow s^{(m)}_T
  \end{split}
\end{equation}
and then fit a model
\begin{equation}
  \mathop{\operatorname{argmin}}_{A,B}\frac{1}{2}\sum_{i=1}^m\sum_{t=1}^{T-1}||s_{t+1}-(As_t+Ba_t)||^2
\end{equation}

Another approach is to linearize a non-linear model $s_{t+1}=f(s_t,a_t)$ around a point $\bar{s}_t, \bar{a}_t$.
\begin{equation}
  s_{t+1}\approx
  f(\bar{s}_t,\bar{a}_t)+
